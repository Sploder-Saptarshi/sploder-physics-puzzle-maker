...
class CxFastAllocList_Callback {
	
	private var head:CxFastNode_Callback;
	private var alloc:Allocator;
	
	
	
	public inline function get():Callback {
		return alloc.CxAlloc_Callback();
	}
	...
	public inline function add(o:Callback) {
		var temp =  {
		var ret = alloc.CxAlloc_CxFastNode_Callback();
		ret.elt = o;
		ret;
	};
		temp.next = begin();
	...
	public inline function addAll(list:CxFastAllocList_Callback) {
		   {
	...
	public inline function pop():Void {
		var ret = begin();
		 head=ret.next;
		 alloc.CxFree_Callback(ret.elem());
		 alloc.CxFree_CxFastNode_Callback(ret);
	}
	public inline function remove(obj:Callback):Bool {
		var pre = null;
		var cur = begin();
		var ret = false;
		while(cur!=end()) {
	...
	public inline function erase(pre:CxFastNode_Callback,cur:CxFastNode_Callback):CxFastNode_Callback {
		var old = cur; cur = cur.next;
		if(pre==null)  head=cur;
		else          pre.next = cur;
		 alloc.CxFree_Callback(old.elem());
		 alloc.CxFree_CxFastNode_Callback(old);
		return cur;
	}
	public inline function splice(pre:CxFastNode_Callback,cur:CxFastNode_Callback,n:Int):CxFastNode_Callback {
		while(n-->0 && cur!=end())
			cur = erase(pre,cur);
		return cur;
	}
	...
	public inline function has(obj:Callback) return  ({
	var ret = false;
	  {
	...
	public inline function insert(cur:CxFastNode_Callback,o:Callback) {
	...
	public inline function free(o:Callback) {}

...
class CxFastList_MPConsPair {
	
	private var head:CxFastNode_MPConsPair;
	...
	public inline function add(o:MPConsPair) {
		var temp =  {
		var ret = alloc.CxAlloc_CxFastNode_MPConsPair();
		ret.elt = o;
		ret;
	};
		temp.next = begin();
	...
	public inline function addAll(list:CxFastList_MPConsPair) {
		   {
	...
	public inline function pop():Void {
		var ret = begin();
		 head=ret.next;
		 {};
		 alloc.CxFree_CxFastNode_MPConsPair(ret);
	}
	public inline function remove(obj:MPConsPair):Bool {
		var pre = null;
		var cur = begin();
		var ret = false;
		while(cur!=end()) {
	...
	public inline function erase(pre:CxFastNode_MPConsPair,cur:CxFastNode_MPConsPair):CxFastNode_MPConsPair {
		var old = cur; cur = cur.next;
		if(pre==null)  head=cur;
		else          pre.next = cur;
		 {};
		 alloc.CxFree_CxFastNode_MPConsPair(old);
		return cur;
	}
	public inline function splice(pre:CxFastNode_MPConsPair,cur:CxFastNode_MPConsPair,n:Int):CxFastNode_MPConsPair {
		while(n-->0 && cur!=end())
			cur = erase(pre,cur);
		return cur;
	}
	...
	public inline function has(obj:MPConsPair) return  ({
	var ret = false;
	  {
	...
	public inline function insert(cur:CxFastNode_MPConsPair,o:MPConsPair) {
	...
	public inline function free(o:MPConsPair) {}

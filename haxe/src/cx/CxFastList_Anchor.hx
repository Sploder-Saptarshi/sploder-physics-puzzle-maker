...
class CxFastList_Anchor {
	
	private var head:CxFastNode_Anchor;
	...
	public inline function add(o:Anchor) {
		var temp =  {
		var ret = alloc.CxAlloc_CxFastNode_Anchor();
		ret.elt = o;
		ret;
	};
		temp.next = begin();
	...
	public inline function addAll(list:CxFastList_Anchor) {
		   {
	...
	public inline function pop():Void {
		var ret = begin();
		 head=ret.next;
		 {};
		 alloc.CxFree_CxFastNode_Anchor(ret);
	}
	public inline function remove(obj:Anchor):Bool {
		var pre = null;
		var cur = begin();
		var ret = false;
		while(cur!=end()) {
	...
	public inline function erase(pre:CxFastNode_Anchor,cur:CxFastNode_Anchor):CxFastNode_Anchor {
		var old = cur; cur = cur.next;
		if(pre==null)  head=cur;
		else          pre.next = cur;
		 {};
		 alloc.CxFree_CxFastNode_Anchor(old);
		return cur;
	}
	public inline function splice(pre:CxFastNode_Anchor,cur:CxFastNode_Anchor,n:Int):CxFastNode_Anchor {
		while(n-->0 && cur!=end())
			cur = erase(pre,cur);
		return cur;
	}
	...
	public inline function has(obj:Anchor) return  ({
	var ret = false;
	  {
	...
	public inline function insert(cur:CxFastNode_Anchor,o:Anchor) {
	...
	public inline function free(o:Anchor) {}

...
class UniformDynamicSpace extends Space {
	
	public var cells:Hash<UniformCell>;
	...
	private inline function hash(x:Int,y:Int) {
		return x+"."+y;
	}
	
	private inline function cellsget(x,y) {
		var ind = hash(x,y);
		var ret = cells.get(ind);
		if(ret==null)
			cells.set(ind, ret = new UniformCell());
		return ret;
	}

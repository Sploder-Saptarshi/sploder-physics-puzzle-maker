...
class Arbiter_false_false_true_true_Shape_Shape extends Arbiter {
	
	public var next:Arbiter_false_false_true_true_Shape_Shape;
	...
	public inline function retire() {
		if(p1!=null) {
			p1.p_arbiters_false_false_true_true_Shape_Shape.remove(this);
			p2.p_arbiters_false_false_true_true_Shape_Shape.remove(this);
		}
		live  = false;
		woken = true;
		updated = false;
		...
		alloc.CxFree_Arbiter_false_false_true_true_Shape_Shape(this);
		stamp = 0;
	}
	...
	public inline function assign(s1:Shape,s2:Shape,ID:Int) {
		...
		p1.p_arbiters_false_false_true_true_Shape_Shape.add(this);
		p2.p_arbiters_false_false_true_true_Shape_Shape.add(this);
	...
	public inline function preStep() {
		var mass_sum = (if(false) p1.smass else 0)
	...
	while(cxiterator != contacts.end()) {
		...
		vrx = (if(false) c.r1y * b1.w else 0)
		     - (if(true) c.r2y * b2.w else 0)
		     - (if(false) p1.vx        else 0)
		     + (if(true) p2.vx        else 0);
		
		vry = (if(true) c.r2x * b2.w else 0)
		     - (if(false) c.r1x * b1.w else 0)
		     - (if(false) p1.vy        else 0)
	...
	while(cxiterator != contacts.end()) {
		...
		if(false)  { p1.vx -= jx*(p1.imass); p1.vy -= jy*(p1.imass); } ;
		if(true)  { p2.vx += jx*(p2.imass); p2.vy += jy*(p2.imass); } ;
		if(false) b1.w -= b1.imoment *  (c.r1x*jy - c.r1y*jx) ;
		if(true) b2.w += b2.imoment *  (c.r2x*jy - c.r2y*jx) ;
	};
		};
		}
		cxiterator = cxiterator.next;
	...
	while(cxiterator != contacts.end()) {
		var c = cxiterator.elem();
		{
			...
			{
			   var vrx:Float;  var vry:Float      ;  {
		vrx = (if(false) c.r1y * b1.bw else 0)
		     - (if(true) c.r2y * b2.bw else 0)
		     - (if(false) p1.bvx        else 0)
		     + (if(true) p2.bvx        else 0);
		
		vry = (if(true) c.r2x * b2.bw else 0)
		     - (if(false) c.r1x * b1.bw else 0)
		     - (if(false) p1.bvy        else 0)
		...
		if(false)  { p1.bvx -= jx*(p1.imass); p1.bvy -= jy*(p1.imass); } ;
		if(true)  { p2.bvx += jx*(p2.imass); p2.bvy += jy*(p2.imass); } ;
		if(false) b1.bw -= b1.imoment *  (c.r1x*jy - c.r1y*jx) ;
		if(true) b2.bw += b2.imoment *  (c.r2x*jy - c.r2y*jx) ;
	};

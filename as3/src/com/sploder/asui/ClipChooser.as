package com.sploder.asui {
    ...
    public class ClipChooser extends Cell {
        ...
        function ClipChooser (container:Sprite, textLabel:String, choices:Array, alts:Array, selectionIndex:Number = 0, promptText:String = "", width:Number = NaN, height:Number = NaN, menuPosition:int = Position.POSITION_RIGHT, position:Position = null, style:Style = null) {
            ...
            init_ClipChooser (container, textLabel, choices, alts, selectionIndex, promptText, width, height, menuPosition, position, style);
        ...
        //
        //
        protected function init_ClipChooser (container:Sprite, textLabel:String, choices:Array, alts:Array, selectionIndex:Number = 0, promptText:String = "", width:Number = NaN, height:Number = NaN, menuPosition:int = Position.POSITION_RIGHT, position:Position = null, style:Style = null):void {
		...
		{
			if (_choices != null) {
				var idx:int = _choices.indexOf(value);
				if (idx >= 0) select(idx);
				else trace("choice " + value + " not found.");
			}
		}

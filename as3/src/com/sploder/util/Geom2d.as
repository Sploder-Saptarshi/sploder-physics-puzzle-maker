...
package com.sploder.util {
	...
	public class Geom2d {
		...
		//
		//
		public static function percentAlongLine (pt:Point, linePtA:Point, linePtB:Point):Number {
			pt = Closest.pointClosestTo(pt, linePtA, linePtB);
			var total:Number = distanceBetween(linePtA, linePtB);
			var delta:Number = distanceBetween(linePtA, pt);
		...
		public static function normalizeAngle (angle:Number):Number {
			...
			if (angle > PI) {
				...
				if (angle < 0.0001 && angle > -0.0001) return 0;
			}
		...
		public static function gridPointsThatIntersectLine (ptA:Point, ptB:Point, gridScale:Number = 1, occludeEnds:Boolean = false):Array {
			...
			if (Math.abs(dx) > Math.abs(dy)) {
				...
				if (dy == 0) dpt = new Point(1, 0);
				else dpt = new Point(1, dy / dx);
				...
				for (i = sx; i <= ex; i++) {
					...
					if (i != ex) cells.push(new Point(pt.x - 1, g));
					else cells.push(new Point(pt.x - 1, rey));
				...
				if (dy == 0) dpt = new Point(0, 1);
				else dpt = new Point(dx / dy, 1);
				...
				for (i = sy; i <= ey; i++) {
					...
					if (i != ey) cells.push(new Point(g, pt.y - 1));
					else cells.push(new Point(rex, pt.y - 1));

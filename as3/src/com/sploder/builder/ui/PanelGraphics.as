package com.sploder.builder.ui {
	...
	public class PanelGraphics extends EventDispatcher {
		...
		public function create (container:Cell):void 
		{
			...
			_contentCell = new Cell(null, 724, 85, true, false, 0, Styles.absPosition.clone( { top: 442, left: 0 } ), Styles.dialogueStyle);
		...
		public function createContent():void 
		{
			...
			_listContainer = new Collection(null, 660, 48, 48, 48, 0, Styles.floatPosition.clone( { margin_top: 5 } ), colStyle);
			...
			_loadingPrompt = new HTMLField(null, "<br /><p align=\"center\">Loading...</p>", 724, true, Styles.absPosition, Styles.dialogueStyle);
			_contentCell.addChild(_loadingPrompt);
			
			_serverMessage = new HTMLField(null, "<br /><p align=\"center\"><b>Server message</b></p>", 724, true, Styles.absPosition, Styles.dialogueStyle);
		...
		public function connect ():void {
			...
			_creator.modelController.selection.addEventListener(Event.CHANGE, onModelSelectionChange);
		...
		protected function onModelSelectionChange (e:Event):void {
			
			_currentSelectionLength = _creator.modelController.selection.length;
				
			if (_currentSelectionLength == 0) {
				toggleConfirmButton(false);
			} else if (_listContainer.selectedMembers.length > 0) {
			...
			switch (e.target) {
				...
				case (_moveButton):
					if (!_moveButton.toggled) {
						_tweener.createTween(_contentCell, "y", _contentCell.y, 442, 0.5, false, false, 0, 0, Tween.EASE_OUT, Tween.STYLE_QUAD);
		...
		protected function onDragFromTray (e:Event):void { 
			_creator.modelController.focusObject = null;
		}
		...
		protected function onDragFromTrayMove (e:Event):void { 
			...
			_creator.modelController.focusObject = _creator.model.objectAtPoint(dropPoint);
		...
		protected function onDropFromTray (e:Event):void { 
			...
			if (_creator.modelController.focusObject == null && _creator.ui.stage.mouseX > 90) {
				_creator.uiController.notice("You can only drop graphics onto objects you've created with the drawing tool!");
				return;
			}
			
			_creator.modelController.history.record();

			if (_creator.modelController.focusObject) {
				if (_creator.modelController.focusObject != null && 
					_creator.modelController.selection.objects.indexOf(_creator.modelController.focusObject) == -1) {
					addGraphic(_creator.modelController.focusObject);
				} else if (_creator.modelController.selection.length <= 1) {
					addGraphic();
				} else {
					_creator.uiController.confirm(
						...
						addGraphic, 
						null,
						"Do you really want to add this graphic as a texture for " + _creator.modelController.selection.length + " objects?"
						);
				}
			}
		...
		public function addGraphic (obj:ModelObject = null):void {
			...
			if (_currentProjectID > 0) {
				...
				if (obj) {
					...
					obj.props.graphic = _currentProjectID;
					obj.props.graphic_version = _currentProjectVersion;
					obj.props.animation = (animate) ? 1 : 0;
					obj.props.texture = 0;
					obj.props.line = -1;
					obj.props.color = -1;
					...
					if (_creator.modelController.selection.length == 1) {
						_creator.ui.animationToggle.enable();
					}
        ...
        protected function addItem (xmlRef:XMLNode, itemtype:String, num:int):CollectionItem {
			...
			//g.addEventListener(Event.COMPLETE, onGIFLoaded);
			var loading:Sprite = Component.library.getDisplayObject(CreatorUIStates.ICON_LOADING) as Sprite;
		...
		protected function onItemRollover (e:Event):void {
			...
			_creator.ui.prompt.show("To use this graphic, drag it onto an object on the canvas.");
			
		}

		protected function onItemRollout (e:Event):void {
			
			_creator.ui.prompt.hide();
		...
		protected function selectProject (projID:String = null):void {
			...
			if (projID != null || _listContainer.selectedMembers.length > 0) {
				...
				if (_currentSelectionLength > 0) toggleConfirmButton(true);
		...
		public function hide():void 
		{
			...
			if (_creator.ui.graphicsPanelToggle &&
				_creator.ui.graphicsPanelToggle.toggled) _creator.ui.graphicsPanelToggle.toggle();
